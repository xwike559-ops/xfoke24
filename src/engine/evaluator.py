...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for i, (img1, img2) in enumerate(test_loader):
            ...
            if i < 5:
                cv2.imwrite(f'result_datasetchanged+_100_v3_epoch{i}.png', cv2.cvtColor(np_imgs[2], cv2.COLOR_RGB2BGR))
                cv2.imwrite(f'resultmask_datasetchanged+_100_v3_epoch{i}.png', mask_colormap)
    return results
